...
// =============================================================================
// VENDOR ITEM — renders one row in either list
// =============================================================================
...
export function VendorBankAssignmentDialog({ bank, open, onClose, onSaved }: Props) {
  ...
  useEffect(() => {
    ...
    fetch(`/api/banks/${bank.id}/vendors`, {
      headers: authHeaders(),
      cache: "no-store",
    })
      .then(async (res) => {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const res = await fetch(`/api/banks/${bank.id}/vendors`, {
        method: "PUT",
        headers: {
  ...
  return (
    <Dialog open={open} onOpenChange={(v) => !v && onClose()}>
      <DialogContent className="max-w-3xl w-full">
        <DialogHeader>
          <DialogTitle>Assign Vendors to Bank</DialogTitle>
          <DialogDescription>
            Manage which vendors are available when{" "}
            <span className="font-medium text-foreground">{bank.bank_name}</span> is selected in
            Write Checks.
          </DialogDescription>
        </DialogHeader>
