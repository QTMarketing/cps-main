...
import { Loader2, ArrowRight, ArrowLeft, AlertCircle, CheckSquare, Square, Building2 } from "lucide-react";
    ...
    >
      ...
      <div className="min-w-0 flex-1">
        <div className="flex items-center justify-between gap-2">
          <span className="font-medium text-sm truncate">{bank.bank_name}</span>
  ...
  return (
    <div className="flex flex-col flex-1 min-w-0">
      <div className="flex items-center justify-between mb-2">
        ...
        <span className="text-xs text-muted-foreground">{banks.length} banks</span>
      </div>
      ...
      <div className="border border-border rounded-md flex-1 overflow-y-auto min-h-[200px] max-h-[320px] p-1 space-y-1 bg-muted/20">
        {filtered.length === 0 ? (
          <div className="flex items-center justify-center h-full py-8 text-sm text-muted-foreground">
            {banks.length === 0 ? "No banks" : "No results"}
          </div>
        ) : (
          filtered.map((bank) => (
            <BankItem
...
export function StoreBankAssignmentDialog({ store, open, onClose, onSaved }: Props) {
  ...
  useEffect(() => {
    ...
    fetch(`/api/stores/${store.id}/banks`, {
      headers: authHeaders(),
      cache: "no-store",
    })
      .then(async (res) => {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const res = await fetch(`/api/stores/${store.id}/banks`, {
        method: "PUT",
        headers: {
      ...
      <DialogContent className="max-w-3xl w-full">
        <DialogHeader>
          <div className="flex items-center gap-2">
            ...
            <DialogTitle>Assign Banks to Store</DialogTitle>
          </div>
          <DialogDescription>
            Manage which bank accounts are available for{" "}
